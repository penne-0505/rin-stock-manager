...
from constants.options import PaymentMethod, FilterOp
...
from repositories.domains.order_repo import OrderRepository, OrderItemRepository
...
class CartService:
    ...
    async def add_item_to_cart(
        ...
        # 在庫確認
        is_stock_sufficient = await self._check_menu_item_stock(request.menu_item_id, request.quantity, user_id)
        
        # 既存のアイテムがあるかチェック
        existing_item = await self.order_item_repo.find_existing_item(cart_id, request.menu_item_id)
        
        if existing_item:
            # 既存アイテムの数量を更新
            new_quantity = existing_item.quantity + request.quantity
            updated_item = await self.order_item_repo.update(existing_item.id, {
    ...
    async def update_cart_item_quantity(
        ...
        # 在庫確認
        is_stock_sufficient = await self._check_menu_item_stock(order_item.menu_item_id, new_quantity, user_id)
        
        # 数量と小計を更新
        updated_item = await self.order_item_repo.update(order_item_id, {
            "quantity": new_quantity,
            "subtotal": menu_item.price * new_quantity
        })
...
class OrderService:
    ...
    async def checkout_cart(
        ...
        updated_order = await self.order_repo.update(cart_id, {
        ...
        # 最終金額を計算して更新
        calculation = await self._calculate_order_total(cart_id, request.discount_amount)
        await self.order_repo.update(cart_id, {"total_amount": calculation.total_amount})
    ...
    async def cancel_order(
        ...
        # 注文をキャンセル状態に更新
        canceled_order = await self.order_repo.update(order_id, {
        ...
        if request.menu_item_name:
            ...
            for order in orders:
                ...
                for item in items:
                    ...
                    if menu_item and request.menu_item_name.lower() in menu_item.name.lower():
...
class KitchenService:
    ...
    async def get_active_orders_by_status(
        ...
        active_orders = await self.order_repo.find_by_status_list(active_statuses, user_id)
    ...
    async def get_order_queue(self, user_id: UUID) -> list[Order]:
        """注文キューを取得（調理順序順）"""
        active_orders = await self.order_repo.find_by_status_list([OrderStatus.PREPARING], user_id)
        
        # 調理開始前の注文を優先順位順に並べる
        not_started = [o for o in active_orders if not o.started_preparing_at]
        in_progress = [o for o in active_orders if o.started_preparing_at and not o.ready_at]
    ...
    async def start_order_preparation(self, order_id: UUID, user_id: UUID) -> Order:
        ...
        updated_order = await self.order_repo.update(order_id, {
            "started_preparing_at": datetime.now()
        })
    ...
    async def update_kitchen_status(
        ...
        print(f"Kitchen status updated for user {user_id}: {active_staff_count} staff, notes: {notes}")
        return True
    ...
    async def get_kitchen_workload(self, user_id: UUID) -> dict[str, Any]:
        ...
        not_started_count = len([o for o in active_orders if not o.started_preparing_at])
        in_progress_count = len([o for o in active_orders if o.started_preparing_at and not o.ready_at])
        ready_count = len([o for o in active_orders if o.ready_at and o.status != OrderStatus.COMPLETED])
        ...
        for order in active_orders:
            if not order.ready_at:  # まだ完成していない注文
                ...
                for item in order_items:
                    menu_item = await self.menu_item_repo.find_by_id(item.menu_item_id)
                    if menu_item:
                        total_estimated_minutes += menu_item.estimated_prep_time_minutes * item.quantity
        ...
        for order in queue:
            if not order.started_preparing_at:  # まだ開始していない注文
                ...
                for item in order_items:
                    menu_item = await self.menu_item_repo.find_by_id(item.menu_item_id)
                    if menu_item:
                        total_wait_time += menu_item.estimated_prep_time_minutes * item.quantity
    ...
    async def optimize_cooking_order(self, user_id: UUID) -> list[UUID]:
        """調理順序を最適化（注文IDリストを返す）"""
        not_started_orders = await self.order_repo.find_by_status_list([OrderStatus.PREPARING], user_id)
        not_started_orders = [o for o in not_started_orders if not o.started_preparing_at]
    ...
    async def predict_completion_times(self, user_id: UUID) -> dict[UUID, datetime]:
        ...
        for order in active_orders:
            estimated_time = await self.calculate_estimated_completion_time(order.id, user_id)
            if estimated_time:
    ...
    async def get_kitchen_performance_metrics(
        ...
        """キッチンパフォーマンス指標を取得"""
        # 指定日の完了注文を取得
        completed_orders = await self.order_repo.find_completed_by_date(target_date, user_id)
    ...
    async def _check_menu_item_stock(self, menu_item_id: UUID, quantity: int, user_id: UUID) -> bool:
        """メニューアイテムの在庫充足を確認"""
        # レシピを取得
    ...
    async def _calculate_order_total(self, order_id: UUID, discount_amount: int = 0) -> OrderCalculationResult:
        """注文の金額を計算"""
        order_items = await self.order_item_repo.find_by_order_id(order_id)
    ...
    async def _consume_materials_for_order(self, order_items: list[OrderItem], user_id: UUID) -> None:
        """注文に対する材料消費を実行"""
        material_consumption = defaultdict(Decimal)
        
        # 必要な材料量を集計
        for item in order_items:
            recipes = await self.recipe_repo.find_by_menu_item_id(item.menu_item_id, user_id)
            for recipe in recipes:
        ...
        for material_id, consumed_amount in material_consumption.items():
            ...
            if material:
                ...
                await self.material_repo.update_stock_amount(material_id, new_stock, user_id)
    
    async def _restore_materials_from_order(self, order_items: list[OrderItem], user_id: UUID) -> None:
        """注文キャンセル時の材料在庫復元"""
