...
class AnalyticsService:
    ...
    async def get_real_time_daily_stats(
        ...
        # 指定日の注文数を取得
        status_counts = await self.order_repo.count_by_status_and_date(target_date, user_id)
        ...
        average_prep_time = int(sum(prep_times) / len(prep_times)) if prep_times else None
    ...
    async def get_popular_items_ranking(
        ...
        """人気商品ランキングを取得"""
        # 売上集計を取得
        sales_summary = await self.order_item_repo.get_menu_item_sales_summary(days, user_id)
    ...
    async def get_hourly_order_distribution(
        ...
        """時間帯別注文分布を取得"""
        # 指定日の全注文を取得
        orders = await self.order_repo.find_by_date_range(target_date, target_date, user_id)
    ...
    async def calculate_revenue_by_date_range(
        ...
        for order in completed_orders:
            date_key = order.completed_at.date().isoformat() if order.completed_at else order.ordered_at.date().isoformat()
    ...
    async def get_material_consumption_analysis(
        ...
        # 材料の消費取引を取得
        transactions = await self.stock_transaction_repo.find_by_material_and_date_range(
        ...
        for tx in consumption_transactions:
            date_key = tx.created_at.date().isoformat() if tx.created_at else datetime.now().date().isoformat()
    ...
    async def calculate_menu_item_profitability(
        ...
        for item in order_items:
            date_key = item.created_at.date().isoformat() if item.created_at else datetime.now().date().isoformat()
    ...
    async def get_daily_summary_with_trends(
        ...
        # トレンド計算
        avg_daily_revenue = comparison_revenue["total_revenue"] / comparison_days if comparison_days > 0 else 0
        revenue_trend = ((target_stats.total_revenue - avg_daily_revenue) / avg_daily_revenue * 100) if avg_daily_revenue > 0 else 0
        
        avg_daily_orders = comparison_revenue["total_orders"] / comparison_days if comparison_days > 0 else 0
        order_trend = ((target_stats.completed_orders - avg_daily_orders) / avg_daily_orders * 100) if avg_daily_orders > 0 else 0
