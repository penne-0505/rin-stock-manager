...
class PurchaseRepository(CrudRepository[Purchase, UUID]):
    ...
    async def find_by_date_range(
        ...
        # 日付を正規化
        date_from_normalized = date_from.replace(hour=0, minute=0, second=0, microsecond=0)
        date_to_normalized = date_to.replace(hour=23, minute=59, second=59, microsecond=999999)
...
class StockAdjustmentRepository(CrudRepository[StockAdjustment, UUID]):
    ...
    async def find_recent(self, days: int, user_id: UUID) -> list[StockAdjustment]:
        ...
        # 期間でフィルタ
        recent_adjustments = [
            adjustment for adjustment in all_adjustments
...
class StockTransactionRepository(CrudRepository[StockTransaction, UUID]):
    ...
    async def find_by_material_and_date_range(
        ...
        filtered_transactions = [
            ...
            if transaction.created_at and date_from_normalized <= transaction.created_at <= date_to_normalized
        ]
        
        # 作成日時で降順ソート
        filtered_transactions.sort(key=lambda x: x.created_at or datetime.min, reverse=True)
    ...
    async def find_consumption_transactions(
        ...
        consumption_transactions = [
            transaction for transaction in all_transactions
            if (transaction.created_at and 
                date_from_normalized <= transaction.created_at <= date_to_normalized and
                transaction.change_amount < Decimal('0'))
        ]
        
        # 作成日時で降順ソート
        consumption_transactions.sort(key=lambda x: x.created_at or datetime.min, reverse=True)
