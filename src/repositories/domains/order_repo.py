...
class OrderRepository(CrudRepository[Order, UUID]):
    ...
    async def search_with_pagination(
        ...
        # データを取得
        orders = await self.find(filters=filter, order_by=order_by, limit=limit, offset=offset)
    ...
    async def find_completed_by_date(
        ...
        # 完了日時で降順
        completed_orders.sort(key=lambda x: x.completed_at or x.ordered_at, reverse=True)
    ...
    async def count_by_status_and_date(
        ...
        target_date_orders = [
            order for order in all_orders
            if date_start <= order.ordered_at <= date_end
        ]
...
class OrderItemRepository(CrudRepository[OrderItem, UUID]):
    ...
    async def find_by_menu_item_and_date_range(
        ...
        filtered_items = [
            ...
            if item.created_at and date_from_normalized <= item.created_at <= date_to_normalized
        ]
    ...
    async def get_menu_item_sales_summary(
        ...
        filtered_items = [
            item for item in all_items
            if item.created_at and start_date_normalized <= item.created_at <= end_date_normalized
        ]
